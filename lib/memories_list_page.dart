  ...
  Future<void> _deleteMemory(Map<String, dynamic> memory) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('حذف الذكرى', textAlign: TextAlign.center, style: TextStyle(color: starWhite, fontSize: 20)),
        content: const Text('هل أنت متأكد من حذف هذه الذكرى؟', textAlign: TextAlign.center, style: TextStyle(color: starWhite, fontSize: 16)),
        actionsAlignment: MainAxisAlignment.spaceEvenly,
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('إلغاء', style: TextStyle(color: lightGray))),
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: Colors.red.shade400), child: const Text('حذف', style: TextStyle(color: Colors.white))),
        ],
      ),
    );
    if (confirmed == true) {
      try {
        ...
        _applyFilter();
        _snack('تم حذف الذكرى');
      } catch (e) { _snack('حدث خطأ'); }
    }
  }
  ...
  Widget _buildSearchBar() {
    return Padding(
      ...
      child: Container(
        ...
        child: TextField(controller: searchController, textAlign: TextAlign.right, onChanged: (_) => _applyFilter(), style: const TextStyle(fontFamily: 'Tajawal', color: starWhite, fontSize: 14), decoration: InputDecoration(hintText: 'ابحث...', hintStyle: TextStyle(fontFamily: 'Tajawal', color: lightGray), prefixIcon: Icon(Icons.search, color: lightGray, size: 20), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12))),
      ),
    );
  }
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(children: [
          ...
          _buildFilterChip('معنونة', 'titled'),
          const SizedBox(width: 6),
          _buildFilterChip('بدون عنوان', 'untitled'),
          const SizedBox(width: 6),
          _buildFilterChip('بها وسائط', 'withMedia'),
          const SizedBox(width: 6),
          _buildFilterChip('نصية فقط', 'textOnly'),
        ]),
      ),
    );
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('لا توجد ذكريات', style: TextStyle(fontFamily: 'Tajawal', fontSize: 16, color: starWhite.withOpacity(0.6))),
        ],
      ),
    );
  }
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Flexible(child: Text(title.isNotEmpty ? title : 'بدون عنوان', overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: 'Tajawal', fontSize: 14, fontWeight: FontWeight.bold, color: title.isNotEmpty ? starWhite : starWhite.withOpacity(0.5)))),
          ]),
                    ...
                    child: _isEditing 
                      ? Container(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(hintText: 'عنوان الذكرى', hintStyle: TextStyle(color: lightGray), border: InputBorder.none),
                          ),
                        )
                      : Text(title.isNotEmpty ? title : 'ذكرى', textAlign: TextAlign.center, overflow: TextOverflow.ellipsis, style: const TextStyle(fontFamily: 'Tajawal', fontSize: 18, fontWeight: FontWeight.bold, color: starWhite)),
                  ),
                  _isEditing
                    ...
                    _isEditing 
                      ? Container(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(hintText: 'اكتب ذكرياتك هنا...', hintStyle: TextStyle(color: lightGray), border: InputBorder.none),
                          ),
                        )
                      : (text.isNotEmpty ? Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: darkGray.withOpacity(0.7), borderRadius: BorderRadius.circular(12), border: Border.all(color: lightGray.withOpacity(0.2))), child: Text(text, textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Tajawal', fontSize: 15, color: starWhite, height: 1.7))) : const SizedBox.shrink()),
