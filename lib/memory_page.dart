      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('إضافة وسائط جديدة', style: TextStyle(fontFamily: 'Tajawal', fontSize: 18, fontWeight: FontWeight.bold, color: starWhite)),
            const SizedBox(height: 20),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                _buildMediaOption(Icons.image, 'صورة', () => Navigator.pop(ctx, 'image')),
                _buildMediaOption(Icons.videocam, 'فيديو', () => Navigator.pop(ctx, 'video')),
              ],
            ),
            const SizedBox(height: 20),
          ],
  ...
  Future<void> _pickAndSaveMedia(String type) async {
    try {
      ...
      if (file != null) {
        ...
        _snack('تم حفظ ${type == "image" ? "الصورة" : "الفيديو"} ✨');
        await _loadAllMedia();
      }
    } catch (e) { _snack('حدث خطأ'); debugPrint('Error: $e'); }
  }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          const Text('استديو', style: TextStyle(color: starWhite, fontSize: 24, fontWeight: FontWeight.bold, letterSpacing: 2)),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          Text('لا توجد وسائط', style: TextStyle(fontFamily: 'Tajawal', fontSize: 18, color: starWhite.withOpacity(0.6))),
          const SizedBox(height: 8),
          Text('اضغط + لإضافة صور أو فيديوهات', style: TextStyle(fontFamily: 'Tajawal', fontSize: 14, color: starWhite.withOpacity(0.4))),
        ],
      ),
    );
  }
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('اختر ذكرى لإضافة الوسائط إليها', style: TextStyle(fontFamily: 'Tajawal', fontSize: 18, fontWeight: FontWeight.bold, color: starWhite)),
            const SizedBox(height: 16),
            Expanded(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Expanded(child: Text(title.isNotEmpty ? title : 'ذكرى بدون عنوان', textAlign: TextAlign.right, style: const TextStyle(fontFamily: 'Tajawal', fontSize: 14, color: starWhite))),
                        ],
                      ),
                    ),
                  );
    ...
    try {
      ...
      if (memoryIndex != -1) {
        ...
        if (pages != null && pages.isNotEmpty) {
          ...
          _snack('تم إضافة ${widget.media["type"] == "image" ? "الصورة" : "الفيديو"} للذكرى ✨');
  ...
  void _confirmDelete() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('حذف', textAlign: TextAlign.center, style: TextStyle(color: starWhite, fontSize: 20)),
        content: const Text('هل تريد حذف هذا الملف؟', textAlign: TextAlign.center, style: TextStyle(color: starWhite, fontSize: 16)),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(title.isNotEmpty ? title : (isVideo ? 'فيديو' : 'صورة'), style: const TextStyle(fontFamily: 'Tajawal', color: starWhite)),
        centerTitle: true,
        actions: [
